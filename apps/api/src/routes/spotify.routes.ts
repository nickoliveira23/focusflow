...
function isAuthenticated(authService: AuthService, request: FastifyRequest) {
  return Boolean(authService.getAuthUserFromRequest(request));
}
...
export async function registerSpotifyRoutes(
  ...
  app.get("/api/spotify/status", async (request, reply) => {
    if (!isAuthenticated(authService, request)) {
